function handleSubmit() {
  ...
  matrixString = matrix
    .map((row, rowIndex) =>
      row
        .map((cell, cellIndex) => `${cell}(row:${rowIndex},col:${cellIndex})`)
        .join(" , ")
    )
    .map((row) => "[ " + row + " ]")
    .join("\n");
  ...
  // Update the content of the div with id="circuitData"
  const circuitDataDiv = document.getElementById("circuitData");
  circuitDataDiv.innerHTML = `<pre>${matrixString}</pre>`;
}
function handleSubmit() {
  const dataTable = document.getElementById("dataTable");
  const matrix = [];
